...
```
6. Modify source files (website-design-related files) to include static files (assets, css, and javascripts) with new paths. In this tutorial, we modify ```src/pug/index.pug```, ```src/scss/sections/_cta.scss```, and ```src/scss/sections/_masthead``` (Click each link to check the modifications). Take ```index.pug``` as example:
```pug
// Favicon
